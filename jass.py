...
def double_dummy_solver3(
  ...
  transposition_table: dict[tuple, TranspositionTableEntry],
    ...
    while playable_cards:
        ...
        hands[curr_player] ^= card
        sub_game_value, sub_game_path = double_dummy_solver0(
            hands=hands,
        ...
        if is_maximizing_player:
            if current_move_value > best_score:
                best_score = current_move_value
                best_path = ((curr_player, card),) + sub_game_path
            alpha = max(alpha, best_score)
        else:
            if current_move_value < best_score:
                best_score = current_move_value
...
def solve_deal(
    hands: list[int], transposition_table: dict[tuple, TranspositionTableEntry] | None = None
) -> tuple[int, tuple]:
