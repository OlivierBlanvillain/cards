...
def test_get_playable_cards():
    ...
    hand = c("K♦,A♣,J♠")
    assert get_playable_cards(c("A♦"), hand) == c("K♦,J♠")

    # players must follow the trump suit
    hand = c("K♦,A♠,J♠")
    assert get_playable_cards(c("6♠"), hand) == c("A♠,J♠")

    # players are never force to play their J♠
    hand = c("K♦,J♠")
    assert get_playable_cards(c("6♠"), hand) == c("K♦,J♠")

    # free to play any cards when unable to follow suit
    hand = c("J♠,7♠,9♣")
    assert get_playable_cards(c("A♥"), hand) == c("J♠,7♠,9♣")

    # free to play lower trump
    hand = c("J♠,8♠,A♣")
    assert get_playable_cards(c("A♥"), hand) == c("J♠,8♠,A♣")
    hand = c("J♠,Q♠")
    assert get_playable_cards(c("9♣"), hand) == c("J♠,Q♠")
    hand = c("9♠,8♠,A♣")
    assert get_playable_cards(c("A♥"), hand) == c("9♠,8♠,A♣")

    # follow suit with the Queen of Hearts or J♠
    hand = c("Q♥,J♠,A♣")
    assert get_playable_cards(c("A♥"), hand) == c("Q♥,J♠")

    # player 3 is void in the led suit (Hearts) and is not forced to trump
    hand = c("9♠,A♣,K♣")
    assert get_playable_cards(c("A♥"), hand) == c("9♠,A♣,K♣")

    # scenario where everything is playable
    hand = c("J♥,9♥,8♣")
    assert get_playable_cards(c("A♦"), hand) == c("J♥,9♥,8♣")
    ...
    assert get_playable_cards(c("A♥"), hand) == c("J♠,7♠,A♣")

    # free to play anything if partner isn't winning
    hand = c("9♠,8♠,Q♥")
    assert get_playable_cards(c("10♣"), hand) == c("9♠,8♠,Q♥")
